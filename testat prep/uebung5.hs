...
foldNat :: b -> (b -> b) -> Nat -> b
foldNat f g Z = f
foldNat f g (S nat) = g (foldNat f g nat)
...
add :: Nat -> Nat -> Nat
add nat1 nat2 = foldNat nat1 S nat2
...
nat2binär :: Nat -> [Bool]
nat2binär nat = foldNat f g nat
  where
    f = [False]
    g [] = [True]
    g (x : xs) = if x == False then (True : xs) else (False : g xs)
...
elemAtIndex idx as
  ...
  | idx > length as = Left "Fehler: Index ist zu gross"
...
convert (BinKnoten a b c) = Knoten a [convert b, convert c]
