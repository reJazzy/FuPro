...
die zwei Listen nach deren lexikographischen 
Größe sortieren (nicht nach deren Inhalt)
...
bagOrd :: Ord a => [a] -> [a] -> Ordering
bagOrd xs ys 
    | fkt (bag xs) 0 < fkt (bag ys) 0 = LT
    | fkt (bag xs) 0 > fkt (bag ys) 0 = GT
    | otherwise = EQ
    where
        fkt [] biggest = biggest
        fkt ((a, b) : acc) biggest 
...
lexProd :: Ord a => [a] -> [a] -> Ordering
lexProd xs ys
    | lengthCompare /= EQ = lengthCompare
    | bagCompare /= EQ = bagCompare
    | otherwise = lexOrd xs ys
    where
